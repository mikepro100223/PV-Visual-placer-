"""Conservative translation of official roof faces onto the analysis image.

Never translate image detections. Accept only a strong, unambiguous boundary
match within four metres; otherwise preserve the official coordinates.
"""
import math
import cv2
import numpy as np
...
def align_roof_faces(faces,image,bounds,max_shift_m=4):
    report=dict(applied=False,east_m=0.0,north_m=0.0,reason='insufficient_boundary_evidence')
    ...
    def response(dx,dy):
        xy=np.rint(points+[dx,dy]).astype(int);x,y=xy[:,0],xy[:,1]
        return np.minimum(np.abs(gx[y,x]*normals[:,0]+gy[y,x]*normals[:,1]),100)
    baseline=response(0,0);base=float(baseline.mean())
    scores=[]
    for dy in range(-radius_y,radius_y+1):
        for dx in range(-radius_x,radius_x+1):
            if math.hypot(dx*sx,dy*sy)<=max_shift_m:
                scores.append((float(response(dx,dy).mean()),dx,dy))
    best,dx,dy=max(scores)
    ...
    alternatives=[score for score,x,y in scores if math.hypot((x-dx)*sx,(y-dy)*sy)>.75]
    ...
    report.update(applied=True,east_m=east,north_m=north,reason='image_boundary_match')
    return [affinity.translate(face,xoff=east,yoff=north) for face in faces],report
