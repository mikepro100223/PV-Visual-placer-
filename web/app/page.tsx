...
type Result = { building_id: number; geojson: FeatureCollection; panel_count: number; additional_kwp: number; annual_kwh: number | null; usable_area_m2: number; existing_pv_area_m2: number; facets: Facet[]; warnings: string[]; image: string };
...
export default function Home() {
  ...
  async function analyze(lat: number, lon: number) {
    ...
    try {
      const data = await request<Result>(`/api/analyze?lat=${lat}&lon=${lon}&confidence=${confidence}&setback=${setback}`);
      ...
      overlay.current = L.geoJSON(data.geojson, {
        style: feature => {
          ...
          return kind === 'panel' ? { color: '#66f5ba', weight: 1, fillColor: '#11bc82', fillOpacity: .55 }
            : kind === 'pv' ? { color: '#55aaff', weight: 2, fillOpacity: .45 }
            : kind === 'obstacle' ? { color: '#ff814f', weight: 2, fillOpacity: .5 }
  ...
  return <main className="workspace">
    <aside className="panel">
      ...
      <div className="settings"><label htmlFor="setback">Roof edge clearance (m)<Input id="setback" type="number" min="0" max="2" step="0.1" value={setback} onChange={e => { const v=Number(e.target.value); if(Number.isFinite(v)) setSetback(Math.min(2,Math.max(0,v))); }} /></label><label htmlFor="confidence">Detection confidence<Input id="confidence" type="number" min="0.1" max="0.9" step="0.05" value={confidence} onChange={e => { const v=Number(e.target.value); if(Number.isFinite(v)) setConfidence(Math.min(.9,Math.max(.1,v))); }} /></label></div>
